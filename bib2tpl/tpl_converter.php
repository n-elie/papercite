...
require_once( dirname( __DIR__ ) . '/' . 'NewBibtexConverter.php' );
 ...
 * @version 2.0
 */
class TplConverter implements NewBibtexConverter {
	/**
	 * BibTex parser
	 *
	 * @access private
	...
	function _translate( $data ) {
		...
		// Now, replace
		return preg_replace_callback( self::$mainPattern, array( $this, "_callback" ), $result );
	}
	...
	function _callback( $match ) {
		...
		if ( $match[1] == "#group" ) {
			...
			foreach ( $this->_data as $groupkey => &$group ) {
				...
				$this->_group                 = &$group;
				$groups                       .= preg_replace_callback( self::$mainPattern, array(
					$this,
					"_callback"
				), $this->group_tpl );
			}
		...
		if ( $match[1] == "#fullentry" ) {
			...
			foreach ( $this->_group as &$entry ) {
				...
				$this->_entry = $entry;
				$entries      .= preg_replace_callback( self::$mainPattern, array(
					$this,
					"_callback"
				), $this->full_entry_tpl );
		...
		if ( $match[1] == "#entry" ) {
			...
			if ( $this->_entry["entrytype"] ) {
				if ( $this->_entry["entrytype"] ) {
					...
					$this->_globals["positionInList"]  = $this->count;
					$t                                 = preg_replace_callback( self::$mainPattern, array(
							$this,
							"_callback"
						), $entryTpl ) . $match[2];
